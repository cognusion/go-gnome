...
//
// This code was taken from https://github.com/gopxl/beep/blob/main/speaker/speaker.go
...
// Additionally, func clamp is func Clamp from https://github.com/gopxl/beep/blob/main/internal/util/math.go which
...
func Init(sampleRate beep.SampleRate, bufferSize int) error {
	...
	// someday that will be ready but we don't care
	go func() {
		<-readyChan
	}()
...
func (s *sampleReader) Read(buf []byte) (n int, err error) {
	...
	for i := range s.buf[:ns] {
		for c := range s.buf[i] {
			...
			//#nosec G115 - byte will wrap the value, and that is needed so the sound is correct.
			low := byte(valInt16)
			//#nosec G115 - byte will wrap the value, and that is needed so the sound is correct.
